...
class QQOpenidBinding(Base):
    ...
    # 按群记录：同一用户每个群一条绑定；群 openid 群唯一（c2c 按用户 openid 唯一）
    __table_args__ = (
        Index(
            "uq_qq_openid_group",
            "group_openid",
            unique=True,
            sqlite_where=text("openid_type = 'group'"),
        ),
        Index(
            "uq_qq_openid_c2c",
            "openid",
            unique=True,
            sqlite_where=text("openid_type = 'c2c'"),
        ),
    )
...
class AIGroup(Base):
    """AI 会话大组（顶层容器）：
    - kind=qq：按 QQ 号归并 —— 该用户所有 openid（各群/私聊）的会话；
    - kind=web：前端大组 —— 用户网页端会话（每用户一个，自动创建）。
    """
    __tablename__ = "ai_groups"

    id = Column(Integer, primary_key=True, autoincrement=True)
    owner_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    kind = Column(String(10), nullable=False, default="web", server_default="web")  # qq | web
    name = Column(String(100), nullable=False)
    qq = Column(String(20), nullable=True, index=True)  # kind=qq 时记录 QQ 号
    created_at = Column(DateTime, default=_now, nullable=False)
    updated_at = Column(DateTime, default=_now, onupdate=_now, nullable=False)
...
class AIFolder(Base):
    """AI 会话分组（组）：
    - QQ 大组下：按 openid 一组（群聊组名=群名、私聊组名=机器人名），openid 非空；
    - 前端大组下：用户自定义分组，openid 为空。
    会话可挂组下，也可直接挂大组下（未分组）。
    """
    ...
    group = relationship("AIGroup", lazy="selectin")
...
class AIConversation(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    owner_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    ai_group_id = Column(Integer, ForeignKey("ai_groups.id"), nullable=True, index=True)
    folder_id = Column(Integer, ForeignKey("ai_folders.id"), nullable=True, index=True)
    ...
    owner = relationship("User", lazy="selectin")
    group = relationship("AIGroup", lazy="selectin")
    folder = relationship("AIFolder", lazy="selectin")
