...
| **#3** | `works_recommend` | 社区运营 | 群内触发「@bot 最近热门 / @bot 推荐小说」，调用站点搜索/热度排行并返回到群聊 |
| **#4** | `works_submit` | 社区运营 | 群消息中识别「安利 xxx」指令 → 调用作品「仅名称模式上传」接口，把聊天记录里的随手安利沉淀为作品 |
...
- **社区运营**
  ...
  - **nonebot2：插件 #3 `works_recommend` + #4 `works_submit`**
    - `@bot 热门 / @bot 搜 xxx` → 调用站点搜索/热度排行。
    - `@bot 安利 <作品名>` → 走 F3「仅名称模式上传」。
