...
- **AI 问答能力已落地（提前实施）**：见 `qqbot/plugins/commands/ai.py` 与 `_lib/aitools.py`。机器人侧已实现：
  - `/ai <问题>`：OpenAI 兼容端点问答（远程 API 或本地模型），多轮对话，可自主联网（文本协议工具调用 `web:search`，依赖本地 SearXNG）；
  - `/ai 重置`：清空当前会话上下文；
  - `/ai 配置`（管理员）：运行时修改 AI 接口参数，持久化到 `data/ai_config.json`，优先于 `.env.prod`；
  - `/搜网 <关键词>`：独立联网搜索命令，只返回结果链接不归纳。
  - 配置变量与部署形态（同机 / Tailscale / 全上云）详见 `qqbot/README.md`「AI 服务的网络部署」与 `qqbot/.env.prod.example` 的 `AI_*` 段。
  - 命令规范详见 [docs/机器人命令规范.md](机器人命令规范.md) §4.8–4.11。
  - **PRD §6.5 插件 #5 `content_moderation`（AI 内容审核）与 #6 `works_suggest`（AI 补全简介/打标签）仍未实现**，只有 AI 问答/联网搜索已提前落地。
...
- **机器人 / AI（扩展）**
  ...
  - **AI 问答能力提前落地**：`/ai` `/ai 重置` `/ai 配置` `/搜网` 已实现（见 §6.6 与 `qqbot/plugins/commands/ai.py`），属独立 AI 命令通道，不依赖 #5/#6 审核与补全接口。
